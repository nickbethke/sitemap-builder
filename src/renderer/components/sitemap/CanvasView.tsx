...
type CanvasViewProps = {
    ...
    onMoveNodeSibling: (id: string, direction: -1 | 1) => void;
    onMoveNodeUpLevel: (id: string) => void;
...
export const CanvasView = forwardRef<CanvasViewHandle, CanvasViewProps>(function CanvasView(props, ref) {
    ...
    const onSizeChange = useCallback((id: string, height: number) => setCardHeights((current) => current[id] === Math.ceil(height) ? current : {
        ...current,
        [id]: Math.ceil(height)
    }), []);
    const layout = useMemo(() => layoutNodes(document.nodes, layoutDirection, Object.fromEntries(Object.entries(cardHeights).map(([id, height]) => [id, {
        width: CARD_WIDTH,
        height
    ...
    useEffect(() => {
        ...
        const wheel = (event: WheelEvent) => {
            ...
            onZoomChange(Math.max(.1, Math.min(2, zoom + (event.deltaY < 0 ? .025 : -.025))));
        };
    ...
    const focusNode = useCallback((nodeId: string) => {
        ...
        canvas.scrollTo({
            left: (position.x + CARD_WIDTH / 2) * zoom - canvas.clientWidth / 2,
            top: (position.y + 75) * zoom - canvas.clientHeight / 2,
            behavior: 'smooth',
        });
    ...
    const navigate = (x: number, y: number) => canvasRef.current?.scrollTo({
        left: x * zoom - canvasRef.current.clientWidth / 2,
        top: y * zoom - canvasRef.current.clientHeight / 2
    });

    return <div className="relative min-h-0">
        <div ref={canvasRef} className="blueprint-canvas absolute inset-0 overflow-auto" onScroll={syncViewport}
             onClick={() => onSelectNode('')}>
            <div style={{width: layout.width * zoom, height: layout.height * zoom}}>
                <div ref={contentRef} className="relative origin-top-left transition-transform"
                    ...
                    <svg
                        ...
                        width={layout.width} height={layout.height}>{document.nodes.map((node) => {
                        const path = connectionPath(node, layout, layoutDirection);
                        return path && <path key={node.id} d={path}/>;
                    })}</svg>
                    {document.nodes.map((node) => <SitemapCard key={node.id} node={node} document={document}
                                                               layout={layout} layoutDirection={layoutDirection}
                                                               selected={selectedId === node.id} draggedId={draggedId}
                                                               dropTarget={dropTargetId === node.id}
                                                               dimmed={Boolean(search && !visibleNodeIds.has(node.id))}
                                                               ...
                                                               canMoveTo={canMoveTo} onDropNode={onDropNode}
                                                               onSizeChange={onSizeChange}/>)}</div>
            </div>
        </div>
        {!presentationMode && document.nodes.length > 0 &&
            <MiniMap layout={layout} nodes={document.nodes} selectedId={selectedId} zoom={zoom} viewport={viewport}
